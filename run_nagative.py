...
scripts_dir = 'C:\\testicashapi\\NegativeTest_Case\\ICPAPI'
...
directories = [
    ('ML201', 'C:\\IcashPost\\NewICPAPI\\Negative Testing\\ML201\\ConsoleApp1\\bin\\Debug\\ConsoleApp1.exe'),
    ('ML202', 'C:\\IcashPost\\NewICPAPI\\Negative Testing\\ML202\\ConsoleApp1\\bin\\Debug\\ConsoleApp1.exe'),
    ('MLNegativeTesting', 'C:\\IcashPost\\NewICPAPI\\Negative Testing\\MLNegativeTesting\\ConsoleApp1\\bin\\Debug\\ConsoleApp1.exe')
]
...
try:
    ...
    # Wait for MLNegativeTesting to finish
    ml_negative_testing_dir, ml_negative_testing_exe = directories[-1]
    process_ml_negative_testing = subprocess.Popen(ml_negative_testing_exe, shell=True)
    time.sleep(10)  # Adjust sleep duration as needed
    keyboard.press_and_release('enter')  # Simulate pressing Enter
    time.sleep(1)  # Wait for 1 second
    keyboard.press_and_release('enter')  # Simulate pressing Enter
    time.sleep(1)  # Wait for 1 second
    keyboard.press_and_release('enter')  # Simulate pressing Enter
    # Wait for MLNegativeTesting process to finish
    process_ml_negative_testing.wait()
...
html_template = ("<html>"
                 ...
                 "</head>"
                 "<body>"
                 "<h1>ICP Negative Test Report</h1>"
                 f"<p><strong>Total Scripts:</strong> {total_count}</p>"
                 f"<p><strong>Passed Scripts:</strong> {pass_count}</p>"
                 f"<p><strong>Failed Scripts:</strong> {fail_count}</p>"
                 f"<p><strong>Pass Percentage:</strong> {pass_percentage:.2f}%</p>"
...
report_dir = r'C:\ProgramData\Jenkins\.jenkins\workspace\ICPAPI\Negativeapireport'
